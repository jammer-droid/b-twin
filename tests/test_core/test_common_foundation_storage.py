...
def test_save_shared_workflow_doc_uses_deterministic_path(tmp_path: Path) -> None:
    ...
    assert path == tmp_path / "entries" / "_global" / "shared" / "workflow" / "2026-03-06" / "epic-001.md"
    assert path.exists()
...
def test_shared_workflow_docs_are_listed_as_indexable_documents(tmp_path: Path) -> None:
    ...
    assert workflow_doc["doc_id"] == "entries/_global/shared/workflow/2026-03-06/epic-001.md"
...
def test_shared_workflow_record_type_is_stable_and_filterable(tmp_path: Path) -> None:
    ...
    assert workflow_docs[0]["path"] == "entries/_global/shared/workflow/2026-03-06/epic-001.md"
